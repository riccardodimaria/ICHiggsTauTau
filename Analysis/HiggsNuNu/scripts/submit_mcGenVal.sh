...
for METCUT in 130 #0 130
  ...
  for CHANNEL in taunu nunu enu munu #taunu mumu
    ...
    for SYST in central #JESUP JESDOWN JERBETTER JERWORSE #NOTE TO RUN JER DOSMEAR MUST BE SET TO TRUE IN THE CONFIG
      ...
      OUTPUTPREFIX=output_gen/ #oldanalysisruns/220713_taunominaltightwithsysts/output/
...
#Process HiggsNuNu_GenVal specific backgrounds
#Signal files and DYtoNuNu
	...
	for FILELIST in `ls filelists/$PRODUCTION/$QUEUEDIR/${PRODUCTION}_MC_*`
	  ...
	  if (( "$?" == 0 )); then
		...
		$JOBWRAPPER "./bin/HiggsNuNu_GenVal --cfg=$CONFIG --filelist="$FILELIST" --input_prefix=$PREFIX --output_name=$WJOB.root --output_folder=$OUTPUTDIR --met_cut=$METCUT $SYSTOPTIONS --channel=$CHANNEL --wstream=$FLAVOUR &> $JOBDIR/$WJOB.log" $JOBDIR/$WJOB.sh
                $JOBSUBMIT $JOBDIR/$WJOB.sh                                                                                      
	      done
	  else  
	      $JOBWRAPPER "./bin/HiggsNuNu_GenVal --cfg=$CONFIG --filelist="$FILELIST" --input_prefix=$PREFIX --output_name=$JOB.root --output_folder=$OUTPUTDIR --met_cut=$METCUT $SYSTOPTIONS --channel=$CHANNEL &> $JOBDIR/$JOB.log" $JOBDIR/$JOB.sh
...
#Process bkg common with HiggsTautau (ONLY NEEDED IN Mar20 and Apr04)
	...
	for FILELIST in `ls filelists/$PRODUCTION/$QUEUEDIR/${FILELISTPREFIX}*`
	  ...
	  if (( "$?" == 0 )); then
		...
		$JOBWRAPPER "./bin/HiggsNuNu_GenVal --cfg=$CONFIG --filelist="$FILELIST" --input_prefix=$SHAREDPREFIX --output_name=$WJOB.root --output_folder=$OUTPUTDIR --met_cut=$METCUT $SYSTOPTIONS --channel=$CHANNEL --wstream=$FLAVOUR &> $JOBDIR/$WJOB.log" $JOBDIR/$WJOB.sh
		$JOBSUBMIT $JOBDIR/$WJOB.sh
	      done
	  else  
	      $JOBWRAPPER "./bin/HiggsNuNu_GenVal --cfg=$CONFIG --filelist="$FILELIST" --input_prefix=$SHAREDPREFIX --output_name=$JOB.root --output_folder=$OUTPUTDIR --met_cut=$METCUT $SYSTOPTIONS --channel=$CHANNEL &> $JOBDIR/$JOB.log" $JOBDIR/$JOB.sh
	      $JOBSUBMIT $JOBDIR/$JOB.sh
	  fi
