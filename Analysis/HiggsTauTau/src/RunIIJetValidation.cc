...
namespace ic {
	...
	RunIIJetValidation::RunIIJetValidation(std::string const& name) : ModuleBase(name) { 
	...
	RunIIJetValidation::~RunIIJetValidation() {
		;
	}

int RunIIJetValidation::PreAnalysis() {
	std::cout << "-------------------------------------" << std::endl;
	std::cout << "RunIIJetValidation" << std::endl;
	std::cout << "-------------------------------------" << std::endl;    
	if (fs_) {
  ...
  if (write_tree_) {
   	outtree_ = fs_->make<TTree>("jetntuple","jetntuple");
        //Electrons:
	...
	int RunIIJetValidation::Execute(TreeEvent *event) {
...
//                std::vector<Tau*> taus = event->GetPtrVec<Electron>("taus");
   ...
   for(UInt_t ielec=0;ielec<jets.size();++ielec){
	...
	int RunIIJetValidation::PostAnalysis() {
		std::cout << "-------------------------------------" << std::endl;
		std::cout << "RunIIJetValidation" << std::endl;
		std::cout << "-------------------------------------" << std::endl;      
		return 0;
	}
	void RunIIJetValidation::PrintInfo() {
		;
	}
}
