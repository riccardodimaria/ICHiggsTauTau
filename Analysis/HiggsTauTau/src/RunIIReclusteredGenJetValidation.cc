...
namespace ic {
	...
	RunIIReclusteredGenJetValidation::RunIIReclusteredGenJetValidation(std::string const& name) : ModuleBase(name) { 
	...
	RunIIReclusteredGenJetValidation::~RunIIReclusteredGenJetValidation() {
		;
	}

int RunIIReclusteredGenJetValidation::PreAnalysis() {
	std::cout << "-------------------------------------" << std::endl;
	std::cout << "RunIIReclusteredGenJetValidation" << std::endl;
	std::cout << "-------------------------------------" << std::endl;    
	if (fs_) {
  ...
  if (write_tree_) {
   	outtree_ = fs_->make<TTree>("reclusteredgenjetntuple","reclusteredgenjetntuple");
        //Electrons:
	...
	int RunIIReclusteredGenJetValidation::Execute(TreeEvent *event) {

		// Get the objects we need from the event
		EventInfo const* eventInfo = event->GetPtr<EventInfo>("eventInfo");
		std::vector<GenJet*> jets = event->GetPtrVec<GenJet>("genJetsReclustered");
//                std::vector<Tau*> taus = event->GetPtrVec<Electron>("taus");
   ...
   for(UInt_t ielec=0;ielec<jets.size();++ielec){
	...
	int RunIIReclusteredGenJetValidation::PostAnalysis() {
		std::cout << "-------------------------------------" << std::endl;
		std::cout << "RunIIReclusteredGenJetValidation" << std::endl;
		std::cout << "-------------------------------------" << std::endl;      
		return 0;
	}
	void RunIIReclusteredGenJetValidation::PrintInfo() {
		;
	}
}
