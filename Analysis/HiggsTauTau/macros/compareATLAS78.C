...
void compareATLAS78() {
  ...
  t_cms->Draw(draw_cms_old, "cms_oldweight", "goff");
  ...
  t_cms->Draw(draw_cms, "cms_weight", "goff");
  ...
  TFile f_atlas("output/lhchcg/HiggsPtTree.root");
  ...
  t_atlas->Draw(draw_atlas, "weight_pT_dyn", "goff");
  ...
  atlas_pt_weighted->SetMarkerStyle(20);
  atlas_pt_weighted->Scale(1. / atlas_pt_weighted->Integral());

  TFile f_atlas7("weights_June18_7TeV.root");
  TH1F *atlas7_pt_weighted = (TH1F*)f_atlas7.Get("atlas_inclusive");
  atlas7_pt_weighted->SetLineWidth(2);
  atlas7_pt_weighted->SetLineColor(kBlack);
  atlas7_pt_weighted->SetMarkerColor(kBlack);
  atlas7_pt_weighted->SetMarkerStyle(20);
  ...
  SetupTwoPadSplitAsRatio(pads, h[0], h[1], "7TeV/8TeV", true, 0.67, 1.33);
  ...
  legend->AddEntry(atlas7_pt_weighted, TString::Format("#splitline{ATLAS 7 TeV Weighted}{mean = %.1f}", atlas7_pt_weighted->GetMean()));
  legend->AddEntry(atlas_pt_weighted, TString::Format("#splitline{ATLAS 8 TeV Weighted}{mean = %.1f}", atlas_pt_weighted->GetMean()));
  FixBoxPadding(pads[0], legend, 0.05);
  legend->Draw();

  TH1F *ratio = reinterpret_cast<TH1F *>(
      MakeRatioHist(atlas7_pt_weighted, atlas_pt_weighted, true, true));
